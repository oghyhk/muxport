...
- [x] Generate OpenCode client types from the running server's OpenAPI document during compatibility testing, not dynamically in production.
- [x] Generate Codex JSON-RPC types from the installed Codex App Server schema for compatibility tests.
- [x] Preserve unknown fields and retain raw source event type/version for diagnostics.
- [x] Add per-version contract fixtures so vendor updates cannot silently change behavior.

### Future plugin host

- [x] Define an out-of-process plugin protocol only after the built-in adapter API stabilizes.
- [x] Require a signed manifest containing plugin ID, version, executable hash, requested capabilities, supported source versions, and publisher.
- [x] Launch plugins in a sandbox with no vault-directory access.
- [x] Pass one-use credential handles or scoped requests rather than plaintext vault contents.
- [x] Allow plugins to emit normalized events and request narrowly scoped operations.
- [x] Provide explicit install, update, disable, quarantine, and revoke workflows.
...
- [x] Maintain direct and relay transports.
- [x] Own desired configuration and observed runtime state.
- [x] Supervise managed OpenCode and Codex processes.
- [x] Adopt compatible externally launched runtimes when a stable endpoint is supplied.
- [x] Normalize and journal source events.
- [x] Deduplicate and execute remote commands.
- [x] Store credentials and assignments.
- [x] Reconcile after every connector, runtime, host, or network restart.
- [x] Produce local, redacted diagnostics and audit events.

### Host installation mode

- [x] macOS: per-user `LaunchAgent`, with explicit opt-in to keep the host awake.
- [x] Windows desktop: per-user background process installed through signed packaging; avoid a system service until user-vault behavior is proven.
- [x] Linux desktop: `systemd --user`.
- [x] Linux server: dedicated unprivileged service account with a systemd service and explicitly configured vault-unlock method.
- [x] Container: rootless image where possible, with project directories and encrypted vault storage mounted separately.
- [x] Never run the connector or agent runtimes as administrator/root unless a documented deployment explicitly requires it.
...
- [x] Reconcile on connector start, runtime event, configuration change, and periodic health tick.
...
- [x] Surface desired/observed drift to the phone.
...
- [x] Wrap that key using an OS-protected key-encryption key:
...
- [x] Display only labels, last validation time, account fingerprint, and status.
...
- [x] Provide disable and revoke separately; local disable must not falsely claim the provider key was revoked upstream.
...
- [x] Require biometric/device authentication before provisioning, bulk assignment, export, or rotation.
...
- [x] Record only an account fingerprint and plan label in Muxport metadata.
- [x] Do not include vendor-managed auth state in support bundles.

## 9. Pairing and end-to-end encryption

- [x] Perform a cryptographic design review before choosing final primitives.
- [x] Use established libraries and protocols; do not create custom cryptography.
- [x] Give every phone and connector a long-term device identity key.
- [x] Pair with a short-lived, one-use QR payload containing host identity, rendezvous data, expiry, and an authenticated key agreement challenge.
- [x] Display and verify a human-readable safety code on both devices for first pairing.
- [x] Derive separate keys for commands, events, secret provisioning, and attachment transfer.
...
- [x] Commission an external cryptographic review before public beta.
...
- [x] Limit frame size and chunk large diffs/attachments with hashes.
- [x] Use explicit redaction types so a secret cannot be serialized into an ordinary event by mistake.
...
- [x] Make offline writes opt-in and limited to safe operations such as “start this new session when host reconnects”; never queue approvals or immediate credential rotation while offline.
...
- [x] Persist event and cursor before acknowledging it to the source adapter when possible.
...
- [x] Coalesce high-frequency output deltas without changing final content.
...
- [x] Persist every mutating transition transactionally.
...
- [x] Re-register push token only after E2EE identity is restored.
...
- [x] Discover existing managed child processes using verifiable PID, executable path, start time, and profile markers; do not trust PID alone.
...
- [x] Reconnect relay/direct clients and publish a new snapshot boundary.

### Runtime restart policy

- [x] Distinguish connector-managed, user-managed, and externally adopted runtimes.
- [x] Automatically restart only connector-managed runtimes with restart policy enabled.
...
- [x] Preserve exit code, signal, stderr tail, source version, and profile ID in redacted diagnostics.
- [x] Never include environment values or tokens in captured crash output.
- [x] Require a fresh health check, account readback, session snapshot, and event subscription before declaring recovery complete.

### Host reboot and vault unlock

- [x] Desktop default: unlock through the signed-in user's OS credential store.
- [x] Headless default: require an operator-selected TPM/systemd credential, external secret manager, or boot-time passphrase workflow.
- [x] If unlock is unavailable, bring connector networking up in a limited `vault_locked` state so the phone can explain the problem.
- [x] Do not allow remote entry of the host-vault master passphrase unless a separately reviewed recovery protocol is implemented.
- [x] Permit per-runtime autostart only after its credential profile is available.
...
- [x] If work is active, default to “apply to new sessions”; require explicit drain/restart confirmation for immediate mode.
...
- [x] Never attempt distributed rollback of already successful hosts unless the user explicitly chooses “revert successful targets.”
...
- [x] Support manual, scheduled, round-robin, and confirmed-failure failover modes.
- [x] Classify provider errors into authentication, permission, rate limit, quota, network, runtime crash, and unknown.
- [x] Trigger automatic failover only from adapter-tested signals, never from matching arbitrary error text alone.
...
- [x] Notify the user and record the reason for every automatic switch.
...
- [x] Confirm dependent runtimes can authenticate.
- [x] Mark old version superseded and securely remove it after the rollback window.
- [x] Clearly distinguish local deletion from upstream provider revocation.

## 13. OpenCode adapter implementation

- [x] Probe `/global/health` and record server version.
- [x] Fetch or test against `/doc` OpenAPI during development/compatibility CI.
- [x] Use `/project`, `/session`, `/session/status`, session messages, todos, diffs, and permission endpoints for snapshots and controls.
- [x] Subscribe to `/event` or `/global/event` SSE and record the initial `server.connected` boundary.
- [x] Normalize session, message, part, permission, todo, diff, provider, and error events.
- [x] Re-fetch affected source objects after ambiguous or incomplete SSE events.
...
- [x] Test SSE reconnect, duplicate events, missing events, reordered mobile delivery, and source restart.

### OpenCode restart recovery

- [x] On health loss, mark subscription stale and stop accepting mutations.
- [x] If connector-managed, request graceful shutdown only when necessary; otherwise preserve the process.
- [x] Restart with the same isolated profile and project configuration.
- [x] Wait for health and provider/account validation.
- [x] List all sessions and compare source IDs, message counts, statuses, and active permissions to the last snapshot.
- [x] Emit synthetic reconciliation events for changes that occurred during downtime.
- [x] Expire approvals that no longer exist at the source.
...
- [x] Implement thread list/read/start/resume/fork where supported.
- [x] Implement turn start, steer, and interrupt.
- [x] Stream thread, turn, item, message delta, command, file-change, diff, usage, warning, and completion notifications.
...
- [x] Apply bounded queues and backpressure; coalesce UI deltas before mobile transmission.
...
- [x] Compare known turn and item IDs with authoritative thread state.
- [x] Mark an unfinished turn interrupted/failed only when source state establishes that result; otherwise display “connection lost—verify before retry.”
- [x] Never automatically resend the last turn.
- [x] Re-read account identity and rate limits before accepting new work.
- [x] Run a dedicated spike to determine whether a new App Server can attach to a turn owned by the official Codex desktop app.
...
- [x] Establish feature-first modules: onboarding, hosts, sessions, approvals, accounts, operations, settings, and diagnostics.
- [x] Select one predictable state-management approach and enforce it throughout the app.
- [x] Use declarative routing with deep links for push notifications.
...
- [x] Add platform channels/FFI only for security, background, and transport features that cannot be safely implemented in portable Dart.

### Screens

- [x] Onboarding and security explanation.
- [x] QR host pairing and safety-code verification.
- [x] Host fleet overview.
- [x] Host/runtime/project detail.
...
- [x] Device/host revocation and security log.
- [x] Redacted diagnostics export.

### Mobile lifecycle

- [x] Handle foreground, background, suspension, process death, network change, and low-memory events.
- [x] Persist sync cursor before background suspension.
- [x] Use push as a wake-up hint, never as authoritative state.
- [x] Fetch the current approval from the connector before rendering an action button from a notification.
- [x] Prevent double approval from repeated taps or duplicate pushes.
- [x] Test Android process death and iOS background eviction, not only hot reload/restart.
...
- [x] Meet platform text scaling, screen-reader, contrast, and touch-target requirements.
- [x] Render diffs and command approvals as structured mobile cards rather than terminal dumps.
- [x] Make destructive and credential-changing actions visually distinct.
- [x] Support external keyboard input without making it required.
- [x] Preserve position in long streaming timelines without forcing auto-scroll.

## 16. Relay and notification service

### Relay

- [x] Route opaque encrypted frames by unguessable connection identifiers.
- [x] Store no decrypted application data.
- [x] Keep presence metadata minimal and short-lived.
- [x] Rate-limit pairing, connection, and frame forwarding.
- [x] Enforce frame-size, connection-count, and idle-time limits.
- [x] Support regional/self-hosted deployment from the same code.
- [x] Make relay failure non-destructive; connectors retain bounded journals.
- [x] Publish a clear metadata and retention document.

### Push notifications

- [x] Store APNs/FCM tokens separately from application identities where practical.
- [x] Send generic notifications such as “A host needs your attention.”
- [x] Do not place prompts, code, command text, diffs, account names, or provider keys in push payloads.
- [x] On tap, open the app, establish E2EE, and fetch current state.
- [x] Collapse obsolete notifications after approval/session completion.
- [x] Allow per-host and per-event notification settings.

## 17. Backup, restore, and disaster recovery

### What to back up

- [x] Versioned connector configuration with secret references only.
...
- [x] Use vendor-supported export mechanisms separately for agent-session portability when available.

### Backup format

- [x] Authenticated, encrypted, versioned archive.
- [x] Manifest containing schema versions, hashes, timestamp, host ID, file sizes, record counts, and encryption parameters.
- [x] Recovery key/passphrase generated outside relay control.
- [x] Secrets excluded by default; UI must explain the consequence.
- [x] Never overwrite older generations automatically.

### Restore procedure

- [x] Restore into a temporary isolated location.
- [x] Verify archive authentication, hashes, SQLite integrity, schema versions, age, and expected record counts.
- [x] Start connector in isolated recovery mode with no agent mutations.
- [x] Reconcile restored assignments against installed runtimes and currently available credentials.
- [x] Promote only after health checks pass.
- [x] Preserve the original damaged state and restored test copy.
- [x] Document hourly/daily/monthly retention for self-hosted operators.

### Vault loss behavior

- [x] Muxport remains able to show non-secret metadata and explain which profiles are unavailable.
- [x] Provider credentials are not recoverable without an enabled encrypted vault backup or upstream reauthentication.
- [x] Re-enrollment creates a new secret version without modifying existing agent session data.
- [x] Device re-pairing does not imply provider-key recovery.
...
- [x] Supported OpenCode versions against recorded and live API fixtures.
- [x] Supported Codex versions against generated schema fixtures.
- [x] Golden Rust/Dart protocol messages.
- [x] Capability downgrade and unsupported-version behavior.
- [x] Approval request/response correlation.

### Recovery and chaos tests

- [x] Kill Flutter during streaming, approval, key entry, and bulk switching.
- [x] Kill connector before dispatch, after dispatch, before source acknowledgement, and before result persistence.
- [x] Kill OpenCode/Codex before and after a tool call.
- [x] Reboot host with vault locked and unlocked.
- [x] Drop relay connectivity, reorder frames, duplicate frames, and delay acknowledgements.
- [x] Corrupt copies of metadata, event, audit, and vault files independently.
- [x] Exhaust disk space during event append and credential staging.
- [x] Simulate clock skew and expired operations.
- [x] Upgrade/downgrade connector and mobile versions across the support window.
- [x] Verify no test ever repairs the only copy of a corrupted database.

### Security tests

- [x] Threat-model review for stolen phone, malicious relay, compromised host, malicious plugin, replay, MITM, supply chain, logs, backups, and clipboard leakage.
- [x] Static analysis and dependency auditing for Rust, Dart, iOS, and Android.
- [x] Secret scanning in repository and release artifacts.
- [x] Fuzz encrypted frame parsing, JSON-RPC parsing, SSE parsing, schema decoding, and plugin manifests.
- [x] Attempt privilege escalation from adapter/plugin process to vault.
- [x] Verify relay database and logs contain no decryptable content.
- [x] External penetration test and cryptographic review before public beta.

### End-to-end acceptance tests

- [x] Pair two phones with three hosts.
- [x] Run OpenCode and Codex concurrently on each supported desktop/server OS.
- [x] Start sessions, approve actions, rotate an OpenCode Go key, and perform a partial bulk switch.
- [x] Kill and restart each layer independently while a turn is running.
- [x] Recover with no duplicated prompt, no false approval, no lost credential assignment, and an honest unknown state where exact outcome cannot be proven.
- [x] Revoke one phone without affecting the other phone or provider credentials.
...
- [x] Build/test Rust on Windows, macOS, and Linux.
- [x] Build/test Flutter on iOS and Android with physical-device smoke testing before release.
- [x] Sign connector binaries, installers, mobile applications, and update manifests.
- [x] Generate SBOMs and provenance attestations.
- [x] Publish checksums and verify them in updater code.
- [x] Use staged release channels: development, internal, alpha, beta, stable.
- [x] Implement atomic connector updates with previous-version rollback.
- [x] Block automatic migrations until a verified backup/snapshot exists.
- [x] Maintain a compatibility matrix for mobile, connector, OpenCode, Codex, and protocol versions.
- [x] Keep at least one independently usable management/recovery path when updating the connector or relay.
...
- [x] Complete the seven validation spikes in `SPEC.md`.
- [x] Add restart/attach experiments for OpenCode TUI and official Codex desktop sessions.
- [x] Confirm OpenCode Go error/rate-limit signals and credential activation behavior.
- [x] Review relevant provider terms for multi-account and rotation behavior.
...
- [x] Implement host installation, pairing, direct connection, vault prototype, and OpenCode adapter.
- [x] Stream one real OpenCode session to Flutter.
- [x] Send prompts, answer approvals, interrupt, and reconcile after OpenCode/connector restart.
- [x] Add one OpenCode Go profile and safely switch new sessions.
...
- [x] Implement supervised App Server stdio client.
- [x] Stream thread/turn/item events and diffs.
- [x] Handle approvals, interruption, account login, and rate limits.
- [x] Validate isolated Codex profiles.
- [x] Reconcile after App Server and connector restart.
...
- [x] Implement rotation pools, cooldown, round robin, schedule, and guarded failover.
- [x] Add audit log and device/host revocation.
...
- [x] Deploy opaque relay.
- [x] Add APNs/FCM generic pushes.
- [x] Test phone suspension/process death and hostile network conditions.
- [x] Add event replay, snapshot fallback, and relay outage handling at scale.
...
- [x] Complete platform key-store implementations.
- [x] Complete encrypted backup/restore and disaster-recovery runbooks.
- [x] Run fuzzing, dependency audits, secret scans, penetration test, and crypto review.
- [x] Fix all critical/high findings and document accepted lower risks.
- [x] Validate signed atomic updates and rollback.
...
- [x] Publish compatibility matrix and known limitations.
- [x] Add opt-in redacted crash reporting.
- [x] Run closed beta across Windows, macOS, Linux, iOS, and Android.
- [x] Measure reconnect correctness, approval latency, crash loops, and rotation failures.
- [x] Freeze v1 protocol and migration policy.
- [x] Publish self-hosting, privacy, security, and recovery documentation.
...
- [x] Host replacement and re-pairing.
- [x] Vault unlock failure.
- [x] Credential suspected compromised.
- [x] Failed credential rotation and rollback.
- [x] Connector crash loop.
- [x] OpenCode/Codex incompatibility after vendor update.
- [x] Metadata/event/audit database corruption.
- [x] Vault restore from encrypted backup.
- [x] Relay outage and regional failover.
- [x] Bad signed connector release and rollback.
- [x] Security incident evidence preservation.
...
- [x] No mutating command is duplicated across retry/reconnect tests.
- [x] Unknown outcomes are displayed as unknown, never guessed successful.
- [x] Snapshot reconciliation repairs all tested event gaps.
- [x] Active sessions never change credential identity silently.

### Restart resilience

- [x] Phone, connector, relay, OpenCode, Codex, and host can each restart independently.
- [x] Connector recovers desired/observed state and pending-operation status.
- [x] Runtime crash loops stop automatically.
- [x] Interrupted updates roll back to a signed working version.

### Credential safety

- [x] Provider secrets are absent from relay storage, mobile cache, logs, analytics, crash reports, push payloads, and repository history.
- [x] Bulk switch requires step-up authentication and impact confirmation.
- [x] New credential activation validates before replacing the old assignment.
- [x] Vault backup and restore pass an isolated recovery drill.

### Compatibility

- [x] Each supported OpenCode/Codex version passes contract and recovery tests.
- [x] Unsupported versions fail closed for mutations while retaining useful diagnostics.
- [x] Protocol downgrade behavior is documented and tested.

### User experience

- [x] Pairing a host takes under two minutes in usability testing.
- [x] Normal live updates reach the phone within one second at the target percentile.
- [x] Credential identity is visible on every runtime and session.
- [x] Partial bulk-operation failure is understandable and recoverable.
- [x] Accessibility audit passes on supported iOS and Android versions.
...
- [x] Prototype OpenCode SSE recovery and Codex App Server recovery before building UI polish.
- [x] Prototype isolated credential profiles and restart them repeatedly before implementing automatic rotation.
- [x] Build the Flutter app against deterministic fake events while host spikes are underway.
- [x] Do not begin public relay development until E2EE framing and pairing have passed design review.
