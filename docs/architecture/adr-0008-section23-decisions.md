...
- **Status:** Accepted
- **Date:** 2026-07-30
- **Context:** `PLAN.md` Section 23 listed 10 key architectural and policy choices that require formal decisions prior to v1 delivery.
...
6. **Vendor Profile Isolation:** OpenCode runtimes are isolated via distinct state/config directories (`~/.muxport/profiles/opencode/<profile_id>/`); Codex runtimes are isolated via `CODEX_HOME` state directories (`~/.muxport/profiles/codex/<profile_id>/`).
...
10. **Minimum Platform Support:** Windows 10+, macOS 12+, Linux (glibc 2.31+), iOS 15+, Android 8+ (API 26+), OpenCode 1.0+, Codex App Server 0.1+.

## Consequences

- Eliminates ambiguity across host vault management, protocol generation, session adoption, and client state isolation.
