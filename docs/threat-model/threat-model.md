...
| Host Memory / Disk Leakage | Credential and vault envelopes use ChaCha20-Poly1305. The long-term host Ed25519 seed is stored in Windows Credential Manager, macOS Keychain Services, or Linux Secret Service with zeroized temporary record buffers and no plaintext fallback. Provider-vault KEK wrapping still requires the same OS integration. | Mock-backend identity tests and a locked-store daemon smoke pass. Provider-vault OS wrapping, headless unlock, platform integration, log redaction, artifact scanning, and crash-report tests remain required. |
