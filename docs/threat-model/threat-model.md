...
| Man-in-the-Middle (MITM) | Ed25519-signed device/host handshake claims bind fresh X25519 keys, identities, route, nonce, challenge, and prior-message hash. The direct endpoint's initial signed challenge additionally binds the pinned host identity, connector boot epoch, and validity window. HKDF and the SAS are bound to the complete signed transcript. Rendezvous tokens are 256-bit, stored only as hashes, claimed once transactionally, and require phone plus host SAS confirmation. | Tamper, signed-challenge epoch/expiry, wrong-challenge, token replay/expiry/cancellation, one-sided confirmation, restart finalization, revoked/unregistered-device, wrong-host, non-contributory-key, and encrypted direct-probe tests pass. Physical QR/SAS UI and network fuzzing remain unimplemented. |
