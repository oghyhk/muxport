...
| Host Memory / Disk Leakage | Credential vault v2 uses a random zeroizing DEK wrapped by a separate KEK. Domain-separated ChaCha20-Poly1305 contexts bind the envelope to the host and each secret to host/profile/provider/type. The long-term host Ed25519 seed is stored in Windows Credential Manager, macOS Keychain Services, or Linux Secret Service with no plaintext fallback. | Wrong-host/KEK, context-swap, v1 migration, rollback, permission, mock identity-backend, and locked-store daemon tests pass. Provider-vault OS KEK wrapping, headless unlock, platform integration, log redaction, artifact scanning, and crash-report tests remain required. |
